...
def cli_parse(cmdargs=None):
    psr = argparse.ArgumentParser(
        ...
        description="Aeon-ZTP bootstrapper for ArcOS",
        add_help=True)
...
class CumulusBootstrap(object):
    ...
    def wait_for_onie_rescue(self, countdown, poll_delay, user='root'):
        """Polls for SSH access to Arrcus device in ONIE rescue mode.
        ...
        while countdown >= 0:
            try:
                msg = 'ArcOS/ONL installation in progress. Waiting for boot to ONIE rescue mode. Timeout remaining: {} seconds'.format(countdown)
    ...
    def do_onie_install(self, user='root'):
        ...
        msg = 'ArcOS download and verification in progress.'
    ...
    def install_os(self):
        ...
        msg = 'Installing ArcOS image=[%s] ... this can take up to 30 min.' % self.image_name
        self.log.info(msg)
        self.post_device_status(message=msg, state='OS-INSTALL')

        install_command = 'sudo  onl-update-from-installer {image_name}'.format(image_name=self.image_name)
